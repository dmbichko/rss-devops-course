    ...
    aws ssm send-command \
      ...
      --parameters '{
        "commands": [
          ...
          "sudo -u ubuntu scp -o StrictHostKeyChecking=no -i /home/ubuntu/.ssh/id_rsa ubuntu@${aws_instance.ec2-k3s_server.private_ip}:/etc/rancher/k3s/k3s.yaml /home/ubuntu/.kube/config",
          "if [ $? -eq 0 ]; then",
          "  sudo sed -i \"s/127.0.0.1/${aws_instance.ec2-k3s_server.private_ip}/g\" /home/ubuntu/.kube/config",
